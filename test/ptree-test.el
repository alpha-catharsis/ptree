...
(ert-deftest ptree-test-iter-add-value ()
  (let* ((pt (list "test" nil nil))
         (iter (ptree-iter pt)))
    (ptree-iter-add-value iter 'zero 0)
    (should (equal pt '("test" nil (zero 0)))))
  (let* ((pt (list "test" nil '("zero" nil nil)))
         (iter (ptree-iter pt)))
    (ptree-iter-add-value iter 'zero 0)
    (should (equal pt '("test" nil (zero 0) ("zero" nil nil)))))
  (let* ((pt (list "test" nil '("zero" nil nil)))
         (iter (ptree-iter pt)))
    (ptree-iter-add-value iter "zzz" 0)
    (should (equal pt '("test" nil ("zero" nil nil) ("zzz" 0)))))
  (let* ((pt (list "test" nil '("zero" nil nil)))
         (iter (ptree-iter pt)))
    (ptree-iter-add-value iter "zero" 0)
    (should (equal pt '("test" nil ("zero" 0))))))
